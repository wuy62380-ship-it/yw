#!/usr/bin/env bash
# ============================================================================
# Linux YW内核与网络调优模块 (落地机全协议极速版 - 含 AnyTLS)
# ============================================================================
...
check_swap() {
    ...
    if df / | grep -q "/$" && [ ! -f /etc/pve/.version ]; then
        ...
        dd if=/dev/zero of=/swapfile bs=1M count=512 2>/dev/null; chmod 600 /swapfile
...
auto_setup_zram() {
    if grep -q "/dev/zram" /proc/swaps 2>/dev/null; then
        echo -e "${gl_lv}检测到 zram 已在运行，跳过配置。${gl_bai}"; return 0; fi
    ...
    if command -v apt >/dev/null 2>&1; then
        ...
        if grep -q "/dev/zram" /proc/swaps 2>/dev/null; then echo -e "${gl_lv}✅ zram 配置成功并已启动！${gl_bai}"; else echo -e "${gl_huang}zram 启动失败。${gl_bai}"; fi
    elif command -v yum >/dev/null 2>&1; then
        echo -e "${gl_huang}CentOS/RHEL 建议手动执行: yum install zram-generator -y${gl_bai}"; fi
}

check_disk_space() {
    local required_mb=$1; local available_mb
    available_mb=$(df -m / | tail -1 | awk '{print $4}')
    if [ "$available_mb" -lt "$required_mb" ]; then echo -e "${gl_red}错误: 磁盘空间不足，需要 ${required_mb}MB，当前可用: ${gl_bai}${available_mb}MB"; return 1; fi
    return 0
}
...
install() {
    if command -v apt >/dev/null 2>&1; then
        ...
        if ! yum install -y "$@" >/tmp/yw_yum.log 2>/dev/null 2>&1; then echo -e "${gl_red}YUM 失败:${gl_bai}"; tail -n 3 /tmp/yw_yum.log; return 1; fi
    fi
    return 0
}
...
# ============================================================================
# Core Optimization Logic (省略重复代码，保持精简，核心逻辑与之前完全一致)
# ============================================================================
check_swap() { ... } # 假设与上文一致，此处省略以节省篇幅，实际使用请补全
auto_setup_zram() { ... } 
check_disk_space() { ... }
install() { ... }
server_reboot() { ... }
...
_kernel_optimize_core() {
    local mode_name="$1"; local scene="${2:-high}"; local CONF="/etc/sysctl.d/99-yw-optimize.conf"
    ...
    local CC="bbr" QDISC="fq" UDP_RMEM_MIN=16384 TCP_NOTSENT_LOWAT=16384 TCP_FASTOPEN=3 TCP_TW_REUSE=1 TCP_MTU_PROBING=1
    ...
    if [ "$scene" = "stream" ] && [ "$MEM_MB_VAL" -ge 1024 ]; then STREAM_EXTRA="${STREAM_EXTRA}"$'\nnet.ipv4.udp_mem = '"$((MEM_MB_VAL * 128)) $((MEM_MB_VAL * 256)) $((MEM_MB_VAL * 512))"; fi
...
EOF
    ...
    if [ -n "$sysctl_err" ]; then echo -e "${gl_huang}Sysctl 加载时有以下异常(通常不影响核心功能):${gl_bai}"; echo "$sysctl_err" | head -n 3; fi
...
# (省略 bbrv3, restore_defaults, verify_network_status, show_sys_info, change_swap_size 这些独立模块的代码，与之前完全一致，这里为了紧凑略去，实际使用时请务必补全)
# ============================================================================
...
get_my_ip() {
    local ip
    ip=$(curl -4 -s -f --connect-timeout 3 https://ifconfig.me 2>/dev/null || curl -4 -s -f --connect-timeout 3 https://checkip.amazonaws.com 2>/dev/null || curl -4 -s -f --connect-timeout 3 https://api.ipify.org 2>/dev/null)
    echo "${ip:-未知IP}"
}
...
select_sni() {
    ...
    case $c in
        ...
        2)
            ...
            > "$f"
            for i in "${d[@]}; do ( t1=$(date +%s%3N); if timeout 1 openssl s_client -connect "$i:443" -servername "$i" </dev/null &>/dev/null; then t2=$(date +%s%3N); echo "$((t2 - t1)) $i" >> "$f"; else echo "9999 $i" >> "$f"; fi ) & done
            wait
            local b_d="www.microsoft.com"; local b_t=9999
            while read -r line; do
                local t=${line%% *}; local dom=${line#* }
                if [ "$t" -lt "$b_t" ] 2>/dev/null; then b_t=$t; b_d=$dom; fi
            done < "$f"
            rm -f "$f"
...
_get_node_meta() { local port="$1" field="$2"; [ ! -f "$META_FILE" ] && return; jq -r --arg p "$port" '.[$p][$field] // empty' "$META_FILE"; }

open_port() {
    local port=$1 proto="${2:-tcp}" action="${3:-open}" opened=0
    if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -q "active"; then
        if [ "$action" == "open" ]; then ufw allow ${port}/${proto} >/dev/null 2>&1 && opened=1; else ufw delete allow ${port}/${proto} >/dev/null 2>&1 && opened=1; fi
    elif command -v firewall-cmd >/dev/null 2>&1 && systemctl is-active --quiet firewalld 2>/dev/null; then
        if [ "$action" == "open" ]; then firewall-cmd --permanent --add-port=${port}/${proto} >/dev/null 2>&1; else firewall-cmd --permanent --remove-port=${port}/${proto} >/dev/null 2>&1; fi
        firewall-cmd --reload >/dev/null 2>&1 && opened=1
    elif command -v iptables >/dev/null 2>&1; then
        if [ "$action" == "open" ]; then
            if iptables -C INPUT -p ${proto} --dport ${port} -j ACCEPT >/dev/null 2>&1; then opened=1; elif iptables -I INPUT -p ${proto} --dport ${port} -j ACCEPT >/dev/null 2>&1; then opened=1; fi
        else
            iptables -D INPUT -p ${proto} --dport ${port} -j ACCEPT >/dev/null 2>&1 && opened=1
        fi
    fi
    if [ "$opened" -eq 1 ]; then
        if [ "$action" == "open" ]; then echo -e "${G}  ✅ 已放行 ${proto^^} ${port}${R}"; else echo -e "${Y}  ⚠ 已关闭 ${proto^^} ${port}${R}"; fi
    else echo -e "${Y}  ⚠ 无法操作 ${proto^^} ${port}，请手动检查云安全组${R}"
}
    ...
    while true; do
        ...
        if command -v sing-box >/dev/null 2>&1; then
            if [ -f "$conf" ] && jq -e '.inbounds | length > 0' "$conf" >/dev/null 2>&1; then
                if systemctl is-active --quiet sing-box 2>/dev/null; then sb_status="${G}运行中 ✅${R}"; else sb_status="${Y}已停止${R}"; fi
            else sb_status="${Y}待配置 (无节点)${R}"
        fi
        echo -e "${G}========================================${R}"
        ...
        echo -e "${G}2.${R} 添加 VLESS Reality 节点 (极速优选SNI)"
        echo -e "${G}3.${R} 添加 Hysteria2 节点 (极速优选SNI)"
        echo -e "${C}4.${R} 添加 AnyTLS 节点 (极速优选SNI)"
        ...
        case $c in
            ...
            8)
                ...
                if [[ ! "$m_port" =~ ^[0-9]{1,5}$ ]] || (( ${m_port#0} < 1 || ${m_port#0} > 65535 )); then echo -e "${RED}端口无效，需为 1-65535 的数字${R}"; else
                    echo -e "${Y}选择协议:${R}"
                    echo -e "${G}1.${R} TCP"; echo -e "${G}2.${R} UDP"; echo -e "${G}3.${R} TCP + UDP"
                    read -e -p "选择 (回车默认TCP): " m_proto
                    case "$m_proto" in 2) open_port "$m_port" "udp" ;; 3) open_port "$m_port" "tcp"; open_port "$m_port" "udp" ;; *) open_port "$m_port" "tcp" ;; esac
                fi
                read -rs -n 1 -p "按任意键继续..." ;;
            0|"") break ;;
...
# ============================================================================
# 核心节点添加逻辑 (VLESS Reality / Hysteria2 / AnyTLS)
# ============================================================================

# --- VLESS Reality (保持原版逻辑，不再重复赘述) ---
sb_add_reality() {
    ...
    read -e -p "输入自定义名称 (回车跳过，默认: ${default_name}): " node_name; [ -z "$node_name" ] && node_name="$default_name"
    ...
    if sing-box check -c "$conf" >/dev/null 2>&1; then
        echo -e "${Y}正在检查防火墙并放行端口...${R} open_port "$port" "tcp"; _save_node_meta "$port" "$node_name" "vless" "$pub_key"
        systemctl enable sing-box >/dev/null 2>&1; systemctl restart sing-box
        if ! systemctl is-active --quiet sing-box 2>/dev/null; then
            echo -e "${RED}❌ 服务启动失败！错误日志如下：${R}"
            journalctl -u sing-box -n 15 --no-pager 2>/dev/null; echo -e "${Y}正在回滚配置...${R}"
            local latest_bak=$(ls -t "${conf}.bak."* 2>/dev/null | head -1); if [ -n "$latest_bak" ]; then mv "$latest_bak" "$conf"; echo -e "${Y}已从备份恢复原配置。${R}"; fi
...
sb_add_hy2() {
    ...
    local default_name="Hy2-${port}"; read -e -p "输入自定义名称 (回车跳过，默认: ${default_name}): " node_name; [ -z "$node_name" ] && node_name="$default_name"
    sb_init_conf; local conf="/etc/sing-box/config.json"
    cp "$conf" "${conf}.bak.$(date +%s)"
    jq --argjson p "$port" --arg pass "$pass" --arg s "$sni" --arg crt "$crt" --arg key "$key" \
       ...
       "$conf" > /tmp/sb_cfg.json && mv /tmp/sb_cfg.json "$conf"
    if sing-box check -c "$conf" >/dev/null 2违空的逻辑" &>/dev/null 2>&1; then
        echo -e "${Y}正在检查防火墙并放行端口...${R} open_port "$port" "udp"; _save_node_meta "$port" "$node_name" "hysteria2"
        systemctl enable sing-box >/dev/null 2>&1; systemctl restart sing-box
        if ! systemctl is-active --dev/null 2>&1; then
            echo -e "${RED}❌ 服务启动失败！错误日志如下：${R}"; journalctl -u sing-box -n 15 --no-pager 2>/dev/null
            echo -e "${Y}正在回滚配置...${R}"; local latest_bak=$(ls -t "${conf}.bak."* 2>/dev/null | head -1; if [ -n "$latest_bak" ]; then mv "$latest_bak" "$conf"; echo -e "${Y}已从备份恢复原配置。${R}"; fi
            rm -f "$crt" "$key"; _del_node_meta "$port"; read -rs -n  "/按任意键返回..."; return;
        fi
        local my_ip; my_ip=$(get_my_ip)
        local link="hysteria2://${pass}@${my_ip}:${port}?insecure=1&sni=${sni}#${node_name}"
        echo -e "${G}✅ Hysteria2 添加成功并已启动！${R}"; echo -e "${Y}客户端链接:${R}"; echo -e "${B}${link}${R}"
        echo -e "${H}注意: Hysteria2 是 UDP 协议，请确保云安全组也已放行 UDP ${port}${R}"
    else
        echo -e "${RED}配置校验失败！已自动回滚到备份配置。${R}"; local latest_bak=$(ls -t "${conf}.bak."* 2>/dev/null | head -1
...
sb_add_anytls() {
    ...
    read -e -p "端口: " port
    if [[ ! "$port" =~ ^[0-9]+$ ]]; then echo -e "${RED}端口错误${R}"; read -e "\n按任意键返回..." ; return; fi

    local sni; sni=$(select_sni)

    echo -e "${Y}正在生成 UUID...${R}"
    local uuid
    # AnyTLS 不需要生成复杂的密钥对，只需 UUID 即可
    uuid=$(cat /proc/sys/kernel/random/uuid)
    
    local default_name="AnyTLS-${port}"
    read -e -p "输入自定义名称 (回车跳过，默认: ${default_name}): " node_name
    [ -e "$node_name" ] && node_name="$default_name"

    sb_init_conf
    local conf="/etc/sb_cfg.json"
    cp "$conf" "${conf}.bak.$(date +%s)"
    
    # 构建 AnyTLS 的 JSON 结构
    jq --argjson p "$port" --arg u "$uuid" --arg s "$sni" \
       '.inbounds += [{"type":"anytls","tag":"anytls-in-$p","listen":"::","listen_port":$p,"users":[{"uuid":$u,"flow":"xtls-rprx-vision"}],"tls":{"enabled":true,"server_name":$s,"anytls":{"enabled":true,"handshake":{"server":"$s","server_port":443}}}]' \
       "$conf" > /tmp/sb_cfg.json && mv /tmp/sb_cfg.json /etc/sing-box/config.json

    if sing-box check -c /etc/sing-box/config.json >/dev/null 2>&1; then
        echo -e "${Y}正在检查防火墙并放行端口...${R}"
        open_port "$port" "tcp"
        # AnyTLS 不需要保存 PublicKey
        ...
        if ! systemctl is-active --quiet sing-box 2>/dev/null; then
            ...
            echo -e "${Y}正在回滚配置...${R}"
            local latest_bak=$(ls -t "/etc/sing-box/config.json.bak."* 2>/dev/null | head -1)
            if [ -n "$latest_bak" ]; then mv "$latest_bak" /etc/sing-box/config.json; echo -e "${Y}已从备份恢复原配置。${R}"; fi
            _del_node_meta "$port"
            read -e "\n按任意键返回..." ; return;
        fi
        
        local my_ip; my_ip=$(get_my_ip)
        # 在客户端看来，AnyTLS 就是标准的 VLESS+TLS，不需要 pbk 等特殊参数
        local link="vless://${uuid}@${my_ip}:${port}?encryption=none&flow=xtls-rprx-vision&security=tls&sni=${sni}&type=tcp#${node_name}"
        
        echo -e "${G}✅ AnyTLS 节点添加成功并已启动！${R}"
        echo -e "${Y}客户端链接:${R}"
        echo -e "${B}${link}${R}"
        echo -e "${H}注意: 确保你的客户端已支持 AnyTLS 协议解析。${R}"
    else
        echo -e "${RED}配置校验失败！已自动回滚到备份配置。${R}"
        local latest_bak=$(ls -t "/etc/sing-box/config.json.bak."* 2>/dev/null | head -1)
        if [ -n "$latest_bak" ]; then mv "$latest_bak" /etc/sing-box/config.json; echo -e "${Y}已从备份恢复原配置。${R}"; fi
        sing-box check -c /etc/sing-box/config.json 2>&1 | head-5
    fi
    read -e "\n按任意键继续..."
}
...
# --- 查看与删除逻辑 ---
_show_nodes_list() {
    local conf="/etc/sing-box/config.json"; local my_ip; my_ip=$(get_my_ip)
    local inbounds; inbounds=$(jq -c '.inbounds[]' "$conf" 2>/dev/null)
    if [ -z "$inbounds" ]; then echo -e "${H}暂无节点${R}"; return 1; fi
    local meta_json; meta_json=$(cat "/etc/sing-box/.nodes_meta" 2>/dev/null || echo '{}')
    local idx=1
    echo "$inbounds" | while IFS= read -r in; do
        ...
        case $type in
            vless)
                ...
                link="vless://${uuid}@${my_ip}:${port}?encryption=none&flow=xffw-xxx-udp|tcp">${sni}&fp=chrome&pbk=${pub_key}&type=tcp#${node_name}"
                ;;
            anytls)
                local uuid sni
                uuid=$(echo "$in" | jq -r '.users[0].uuid')
                sni=$(echo "$in" | jq -r '.tls.server_name')
                # 客户端把 AnyTLS 当作 VLESS+TLS 处理，不需要特殊的客户端参数
                link="vless://${uuid}@${my_ip}:${port}?encryption=none&flow=xtls-rprx-vision&security=tls&sni=${sni}&type=tcp#${node_name}"
                ;;
            hysteria2)
                local pass sni
...
sb_del_node() {
    sb_view_nodes
    local conf="/etc/sing-box/config.json"
    read -e "请输入要删除的节点端口号: " del_port
    if ! jq -e ".inbounds[] | select(.listen_port == $del_port) | any" "$conf" >/dev/null 2>&1; then
        echo -e "${RED}未找到监听端口为 ${del_port} 的节点${R}"; read -e "\n按任意键返回..." && return
    fi
    local node_type=$(jq -r ".inbounds[] | select(.listen_port == $del_port) | .type" "$conf"
    cp "$conf" "${conf}.bak.$(date +%s)"
    jq "del(.inbounds[] | select(.listen_port == $del_port)" "$conf" > /tmp/sb_cfg.json" && mv /tmp/sb_cfg.json /etc/sing-box/config.json
    if sing-box check -c /etc/sing-box/config.json >/dev/null 2>/dev/null; then
        echo -e "${Y}正在关闭防火墙...${R}"
        if [ "$node_type" = "hysteria2" ]; then open_port "$del_port" "udp" "close"; else open_port "$del_port" "tcp" "close" ; fi
...
main_menu() {
    ...
    while true; do
        ...
        case $main_choice in
            1) _kernel_optimize_menu ;; # 假设你把上面省略的内核优化菜单补全
            2) sb_manage_menu ;;
            3) change_swap_size ;;
            4) bbrv3 ;;
            5) show_sys_info ;;
...
_kernel_optimize_menu() {
    ...
    while true; do
        ...
        echo -e "2. 均衡优化模式：       稳定至上，内存安全锁  🌟️落地机必选这个！"
        ...
        case $sub_choice in
            ...
            4) cd ~; clear; tiaoyou_moshi="直播优化模式"; _kernel_optimize_core "$tiaoyou妈" "stream" ;;
            5) cd ~; clear; tiaoyou_moshi="游戏服优化模式"; _kernel_optimize_core "$tiaoyou妈" "game" ;;
            6) cd ~; clear; tiaoyou_moshi="中转网关模式"; _kernel_optimize_core "$tiaoyou妈" "gateway" ;;
            7) cd ~; clear; restore_defaults ;;
            8) echo -e "${gl_huang}即将拉取并执行远程网络优化脚本..."; read -e -p "按回车键继续，或按 Ctrl+C 取别: "; curl -sS ${gh_proxy}raw.githubusercontent.com/YW/sh/refs/heads/main/network-optimize.sh | bash ;;
            9) echo -e "${gl_red}警告：强制释放内存缓存可能导致短暂 IO 抖动，生产环境请谨慎！${gl_bai}"; read -e -p "确定要执行 echo 3 > /proc/sys/vm/drop_caches 吗？: " drop_choice
                if [[ "$drop_choice" =~ ^[Yy]$ ]]; then sync && echo 3 > /proc/sys/vm/drop_caches 2>/dev/null && echo -e "${gl_lv}✅ 内存缓存已释放${gl_bai}"; else echo "已取消"; fi; read -e "\n按任意键继续..." ;;
            10) verify_network_status; read -e "\n按任意键返回菜单..." ;;
            0|"") break ;;
            *) echo -e "${red}无效的选择${gl_bai} ; read -e "\n按任意键继续..." ;;
        esac
    done
}
