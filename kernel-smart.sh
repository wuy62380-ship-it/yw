...
check_env() {
    local need_update=0
    for pkg in curl jq openssl iptables wget tar python3 ca-certificates; do
    ...
    if [ "$need_update" -eq 1 ]; then
        ...
        apt-get install -y curl jq openssl iptables wget tar python3 ca-certificates >/dev/null 2>&1
...
sb_add_reality() {
    ...
    if $SB_BIN check -c "$SB_CONF" >/dev/null 2>&1; then
        ...
        if systemctl is-active --quiet sing-box; then echo -e "${G}✅ 成功 | SNI: ${sni} | PublicKey: ${pub_key}${R}"; sb_sync_client_config; _persist_iptables; else
...
sb_add_hysteria2() {
    ...
    cp "$SB_CONF" "${SB_CONF}.bak.$(date +%s)"
    local ij=$(jq -n --argjson p "$port" --arg pass "$pass" --arg c "${cert_dir}/cert.pem" --arg k "${cert_dir}/key.pem" '{"type":"hysteria2","tag":("hysteria2-"+($p|tostring)),"listen":"::","listen_port":$p,"users":[{"password":$pass}],"tls":{"enabled":true,"alpn":["h3"],"certificate_path":$c,"key_path":$k}}')
    jq --argjson inb "$ij" '.inbounds += [$inb]' "$SB_CONF" > "$TMP_DIR/sb_cfg.json" && mv "$TMP_DIR/sb_cfg.json" "$SB_CONF"
    if $SB_BIN check -c "$SB_CONF" >/dev/null 2>&1; then
        open_port_both "$port"; _save_node_meta "$port" "$nn" "hysteria2" "" "password=${pass};tls_method=selfsign;sni=www.bing.com"
        systemctl restart sing-box; sleep 2
        if systemctl is-active --quiet sing-box; then echo -e "${G}✅ 成功 | 密码: ${pass}${R}"; sb_sync_client_config; _persist_iptables; else
            echo -e "${RED}启动失败${R}"; local latest_bak=$(ls -t "${SB_CONF}.bak."* 2>/dev/null | head -1); [ -n "$latest_bak" ] && mv "$latest_bak" "$SB_CONF"; _del_node_meta "$port"; fi
    else echo -e "${RED}校验失败${R}"; local latest_bak=$(ls -t "${SB_CONF}.bak."* 2>/dev/null | head -1); [ -n "$latest_bak" ] && mv "$latest_bak" "$SB_CONF"; _del_node_meta "$port"; fi
    _clean_bak; read -rs -n 1 -p ""
}
...
sb_add_tuic() {
    ...
    cp "$SB_CONF" "${SB_CONF}.bak.$(date +%s)"
    local ij=$(jq -n --argjson p "$port" --arg u "$uuid" --arg pass "$pass" --arg c "${cert_dir}/cert.pem" --arg k "${cert_dir}/key.pem" '{"type":"tuic","tag":("tuic-"+($p|tostring)),"listen":"::","listen_port":$p,"users":[{"uuid":$u,"password":$pass}],"congestion_control":"bbr","tls":{"enabled":true,"alpn":["h3"],"certificate_path":$c,"key_path":$k}}')
    jq --argjson inb "$ij" '.inbounds += [$inb]' "$SB_CONF" > "$TMP_DIR/sb_cfg.json" && mv "$TMP_DIR/sb_cfg.json" "$SB_CONF"
    if $SB_BIN check -c "$SB_CONF" >/dev/null 2>&1; then
        open_port_both "$port"; _save_node_meta "$port" "$nn" "tuic" "" "uuid=${uuid};password=${pass};tls_method=selfsign;sni=www.bing.com"
        systemctl restart sing-box; sleep 2
        if systemctl is-active --quiet sing-box; then echo -e "${G}✅ 成功 | UUID: ${uuid} | 密码: ${pass}${R}"; sb_sync_client_config; _persist_iptables; else
            echo -e "${RED}启动失败${R}"; local latest_bak=$(ls -t "${SB_CONF}.bak."* 2>/dev/null | head -1); [ -n "$latest_bak" ] && mv "$latest_bak" "$SB_CONF"; _del_node_meta "$port"; fi
    else echo -e "${RED}校验失败${R}"; local latest_bak=$(ls -t "${SB_CONF}.bak."* 2>/dev/null | head -1); [ -n "$latest_bak" ] && mv "$latest_bak" "$SB_CONF"; _del_node_meta "$port"; fi
    _clean_bak; read -rs -n 1 -p ""
}
...
sb_add_all() {
    ...
    local ij_hy=$(jq -n --argjson p "$p_hy" --arg pass "$hy_pass" --arg c "/etc/sing-box/certs/hy2-${p_hy}/cert.pem" --arg k "/etc/sing-box/certs/hy2-${p_hy}/key.pem" '{"type":"hysteria2","tag":("hysteria2-"+($p|tostring)),"listen":"::","listen_port":$p,"users":[{"password":$pass}],"tls":{"enabled":true,"alpn":["h3"],"certificate_path":$c,"key_path":$k}}')
    local ij_tu=$(jq -n --argjson p "$p_tu" --arg u "$tu_uuid" --arg pass "$tu_pass" --arg c "/etc/sing-box/certs/tuic-${p_tu}/cert.pem" --arg k "/etc/sing-box/certs/tuic-${p_tu}/key.pem" '{"type":"tuic","tag":("tuic-"+($p|tostring)),"listen":"::","listen_port":$p,"users":[{"uuid":$u,"password":$pass}],"congestion_control":"bbr","tls":{"enabled":true,"alpn":["h3"],"certificate_path":$c,"key_path":$k}}')
    ...
    if $SB_BIN check -c "$SB_CONF" >/dev/null 2>&1; then
        ...
        _save_node_meta "$p_hy" "Hysteria2" "hysteria2" "" "password=${hy_pass};tls_method=selfsign;sni=www.bing.com"
        _save_node_meta "$p_tu" "TUIC" "tuic" "" "uuid=${tu_uuid};password=${tu_pass};tls_method=selfsign;sni=www.bing.com"
        systemctl restart sing-box; sleep 2
...
sb_del_node() {
    ...
    if [[ ! "$del_input" =~ ^[0-9]{1,5}$ ]] || [ "$del_input" -lt 1 ] || [ "$del_input" -gt 65535 ]; then
        echo -e "${RED}❌ 输入错误：请输入 1-65535 之间的纯数字端口号！${R}"
        read -rs -n 1 -p "按任意键返回..."
        return
    fi
            ...
            2) 
               ...
               if [[ "$sp" =~ ^[0-9]{1,5}$ ]] && [[ "$ep" =~ ^[0-9]{1,5}$ ]] && [ "$sp" -ge 1 ] && [ "$ep" -le 65535 ] && [ "$sp" -le "$ep" ]; then
                   if command -v ufw >/dev/null 2>&1 && ufw status 2>/dev/null | grep -q "active"; then
                       ...
                       echo -e "${Y}  ⚠ 未检测到支持的防火墙，请手动在云控制台放行。${R}"
                   fi
                   _persist_iptables
               else
                   echo -e "${RED}❌ 端口范围输入错误！请确保起始端口不大于结束端口且在 1-65535 之间。${R}"
               fi
               read -rs -n 1 -p "按任意键继续..." ;;
...
sb_menu() {
    while true; do
        ...
        echo ""
        echo -e "    ${H}[1] 添加 VLESS-Reality (大厂SNI优选)${R}"
        echo -e "    ${H}[2] 添加 VLESS-WS (大厂CDN优选)${R}"
