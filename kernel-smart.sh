...
# 核心重构：直接从 GitHub 下载 deb 包安装，彻底绕开失效的 APT 源
xanmod_download_and_install() {
    echo -e "${Y}正在获取最新 XanMod BBRv3 内核版本...${R}"
    # 从 GitHub API 获取所有的 releases，提取包含 x64v3 的 deb 包链接
    local api_data=$(curl -sL https://api.github.com/repos/xanmod/linux/releases)
    local deb_url=$(echo "$api_data" | jq -r '[.[] | select(.assets != null)] | .[0].assets[] | select(.name | test("linux-image.*x64v3.*amd64.deb$")) | .browser_download_url' | head -n 1)
    
    # 如果找不到 x64v3 架构的包，尝试找通用的 x64 包
    if [ -z "$deb_url" ]; then
        deb_url=$(echo "$api_data" | jq -r '[.[] | select(.assets != null)] | .[0].assets[] | select(.name | test("linux-image.*x64.*amd64.deb$")) | .browser_download_url' | head -n 1)
    fi

    local deb_name=$(basename "$deb_url")

    if [ -z "$deb_url" ]; then
        echo -e "${RED}获取下载链接失败，可能是网络问题或未找到适合的包。${R}"
        read -rs -n 1 -p ""; return 1
    fi

    echo -e "${Y}正在下载 $deb_name ...${R}"
    curl -L -o "/tmp/$deb_name" "$deb_url"
