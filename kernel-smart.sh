...
_wait_for_realm_active() {
    local i=0
    while [ $i -lt 10 ]; do
        if systemctl is-active --quiet realm 2>/dev/null; then return 0; fi
        sleep 0.5; i=$((i+1))
    done
    return 1
}
...
realm_install() {
    if [ -f "$REALM_BIN" ]; then
        echo -e "${Y}Realm 已安装！${R}"; read -rs -n 1 -p ""; return
    fi
    local arch=$(uname -m)
    case "$arch" in
        x86_64) arch="x86_64";;
        aarch64) arch="aarch64";;
        *) echo -e "${RED}❌ 不支持 ${arch}${R}"; return 1;;
    esac
    local latest_ver=$(curl -sL https://api.github.com/repos/zhboner/realm/releases/latest | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/' | sed 's/v//')
    [ -z "$latest_ver" ] && latest_ver="2.6.3"
    echo -e "${Y}正在安装 Realm ${latest_ver} (${arch})...${R}"
    if curl -L -o "$TMP_DIR/realm.tar.gz" -# "https://github.com/zhboner/realm/releases/download/v${latest_ver}/realm-${arch}-unknown-linux-gnu.tar.gz"; then
        tar xzf "$TMP_DIR/realm.tar.gz" -C "$TMP_DIR" 2>/dev/null
        local tmp_bin=$(find "$TMP_DIR" -name "realm" -type f | head -1)
        if [ -n "$tmp_bin" ] && chmod +x "$tmp_bin" && "$tmp_bin" -v >/dev/null 2>&1; then
            mv -f "$tmp_bin" "$REALM_BIN"
            mkdir -p /etc/realm
            cat > "$REALM_CONF" <<EOF
[network]
tcp_timeout = 300
...
EOF
            ...
            echo -e "${G}✅ Realm 安装成功！${R}"
        else
            echo -e "${RED}❌ 下载的执行文件无法运行或解压失败！${R}"
        fi
        rm -rf "$TMP_DIR/realm.tar.gz" "$TMP_DIR/realm"*
    else
        echo -e "${RED}❌ 下载失败${R}"
    fi
    read -rs -n 1 -p ""
...
realm_uninstall() {
    if [ ! -f "$REALM_BIN" ]; then
        echo -e "${Y}Realm 未安装${R}"; read -rs -n 1 -p ""; return
    fi
    read -e -p "确认卸载 Realm？: " c; [[ ! "$c" =~ ^[Yy]$ ]] && return
    systemctl stop realm 2>/dev/null
    systemctl disable realm 2>/dev/null
    rm -rf /etc/realm "$REALM_BIN" "$REALM_SERVICE"
    systemctl daemon-reload 2>/dev/null
    echo -e "${G}✅ Realm 已完全卸载${R}"
    read -rs -n 1 -p ""
}

realm_add_forward() {
    if [ ! -f "$REALM_BIN" ]; then
        echo -e "${RED}请先安装 Realm${R}"; read -rs -n 1 -p ""; return
    fi
    read -e -p "请输入本地监听端口: " l_port
    l_port=$(echo "$l_port" | tr -d '[:space:]')
    [[ ! "$l_port" =~ ^[0-9]+$ ]] && return
    read -e -p "请输入目标服务器IP: " r_ip
    read -e -p "请输入目标服务器端口: " r_port
    r_port=$(echo "$r_port" | tr -d '[:space:]')
    [[ ! "$r_port" =~ ^[0-9]+$ ]] && return

    if grep -q "0.0.0.0:${l_port}" "$REALM_CONF" 2>/dev/null; then
        echo -e "${RED}端口 ${l_port} 已存在于配置中！${R}"
        read -rs -n 1 -p ""; return
    fi

    echo -e "\n[[endpoints]]\nlisten = \"0.0.0.0:${l_port}\"\nremote = \"${r_ip}:${r_port}\"" >> "$REALM_CONF"
    
    open_port_both "$l_port"
    _persist_iptables

    systemctl restart realm
    if _wait_for_realm_active; then
        echo -e "${G}✅ 转发规则添加成功！${R}"
        echo -e "${C}监听: 0.0.0.0:${l_port} -> 远程: ${r_ip}:${r_port}${R}"
    else
        echo -e "${RED}❌ Realm 启动失败，配置可能有误！${R}"
        journalctl -u realm -n 10 --no-pager
    fi
    read -rs -n 1 -p ""
}
...
realm_list_forward() {
    ...
    while IFS= read -r line; do
        if echo "$line" | grep -q "listen"; then
            listen=$(echo "$line" | awk -F'"' '{print $2}')
        elif echo "$line" | grep -q "remote"; then
            local remote=$(echo "$line" | awk -F'"' '{print $2}')
            if [ -n "$listen" ] && [ -n "$remote" ]; then
                echo -e "${G}[$idx] ${listen} -> ${remote}${R}"
                idx=$((idx+1)); has_any=1
            fi
        fi
    done < "$REALM_CONF"
...
realm_del_forward() {
    ...
    realm_list_forward
    read -e -p "请输入要删除的本地监听端口: " del_port
    del_port=$(echo "$del_port" | tr -d '[:space:]')
    [ -z "$del_port" ] && return
    
    local line_num=$(grep -n "0.0.0.0:${del_port}" "$REALM_CONF" | head -1 | cut -d: -f1)
    if [ -z "$line_num" ]; then
        echo -e "${RED}未找到端口 ${del_port} 的转发${R}"; read -rs -n 1 -p ""; return
    fi
    
    cp "$REALM_CONF" "$REALM_CONF.bak"
    ...
    systemctl restart realm
    if _wait_for_realm_active; then
        echo -e "${G}✅ 已删除并重启 Realm${R}"
        del_port_both "$del_port"
    else
        echo -e "${RED}❌ 删除后重启失败，恢复配置${R}"
        mv "$REALM_CONF.bak" "$REALM_CONF"
        systemctl restart realm
    fi
    _persist_iptables
    rm -f "$REALM_CONF.bak"
...
realm_menu() {
    while true; do
        clear
        local realm_status="${H}未安装${R}"
        if [ -f "$REALM_BIN" ]; then
            if systemctl is-active --quiet realm 2>/dev/null; then
                realm_status="${G}● 运行中${R}"
            else
                realm_status="${RED}○ 未运行${R}"
            fi
        fi
        echo -e "${G}╔═══════════════════════════════════════════╗"
        echo -e "║       🔄 Realm 中转管理面板                 ║"
        echo -e "╚═══════════════════════════════════════════╝${R}"
        echo -e "    当前状态: ${realm_status}"
        echo ""
        echo -e "    ${G}[1] 安装 Realm${R}"
        echo -e "    ${G}[2] 添加转发规则${R}"
        echo -e "    ${G}[3] 查看转发列表${R}"
        echo -e "    ${G}[4] 删除转发规则${R}"
        echo -e "    ${H}[5] 重启 Realm${R}"
        echo -e "    ${H}[6] 卸载 Realm${R}"
        echo ""
        echo -e "    ${H}[0] 返回优化中心${R}"
        echo ""
        read -e -p "  请选择: " c
        case "$c" in
            ...
            2) clear; realm_add_forward ;;
            3) clear; realm_list_forward; read -rs -n 1 -p "" ;;
            4) clear; realm_del_forward ;;
            5) clear; systemctl restart realm 2>/dev/null && echo -e "${G}✅ 已重启${R}" || echo -e "${RED}重启失败${R}"; read -rs -n 1 -p "" ;;
            6) clear; realm_uninstall ;;
            0|"") break ;;
            *) echo -e "${RED}无效选择${R}"; sleep 1 ;;
        esac
    done
